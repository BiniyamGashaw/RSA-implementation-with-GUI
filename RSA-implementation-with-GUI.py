...
# title of the window
root.title("RSA implimentation by Biniyam")


# label

Label(root, text='RSA based Message ENCODING and DECODING',
...
Label(root, font='arial 12 bold',
      text='Enter prime number(p)').place(x=60, y=110)
Entry(root, font='arial 10', textvariable=prime1,
      bg='ghost white', borderwidth=5).place(x=290, y=110)
Label(root, font='arial 12 bold',
      text='Enter prime number(q)').place(x=60, y=140)
Entry(root, font='arial 10', textvariable=prime2,
      bg='ghost white', borderwidth=5).place(x=290, y=140)


# Message
Label(root, font='arial 12 bold', text='Enter the Message').place(x=60, y=180)
Entry(root, font='arial 10', textvariable=Text,
      bg='ghost white', borderwidth=5).place(x=290, y=180)
...
# mode
Label(root, font='arial 12 bold',
      text='MODE(e-encode, d-decode)').place(x=60, y=220)
Entry(root, font='arial 10', textvariable=mode,
      bg='ghost white', borderwidth=5).place(x=290, y=220)


# result
Entry(root, font='arial 10 bold', textvariable=Result,
      bg='ghost white', borderwidth=5).place(x=290, y=260)

# result button
Button(root, font='arial 10 bold', text='RESULT', padx=2,
       bg='LightGray', command=Mode).place(x=60, y=260)


# reset button
Button(root, font='arial 10 bold', text='RESET', width=6,
       command=Reset, bg='LimeGreen', padx=2).place(x=60, y=330)

# exit button
Button(root, font='arial 10 bold', text='EXIT', width=6, command=Exit,
       bg='OrangeRed', padx=2, pady=2).place(x=290, y=330)
root.mainloop()
